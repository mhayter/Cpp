...
//slow because of huge memory allocations 1.16s
...
//w,v
const int MAX_ATTACH = 7500;
pair<int,int> adj[SIZE][MAX_ATTACH];
int deg[SIZE];
...
int dijkstraBidirectional(int source, int n,int dest) {
	...
	while (Fque.size() || Bque.size()) {
		if (Fque.size()) {
			if (Bque.top().first + Fque.top().first >= minDist) return minDist;
			Fu = Fque.extract().second;
			
			for(int i=0;i<deg[Fu];++i) {
				v = adj[Fu][i].second;
				int w = adj[Fu][i].first;
				alt = Fdist[Fu] + w;
				
				if (alt < Fdist[v]) {
					Fdist[v] = alt;
					if (Bprocessed[v]) {
						minDist = min(minDist,Fdist[Fu]+Bdist[v]+w);
					}
		...
		if (Bque.size()) { 
			//cerr << "B in" << endl;
			if (Bque.top().first + Fque.top().first >= minDist) return minDist;
			Bu = Bque.extract().second;	
				
			for(int i=0;i<deg[Bu];++i) {
				v = adj[Bu][i].second;
				int w = adj[Bu][i].first;
				alt = Bdist[Bu] + w;
				
				if (alt < Bdist[v]) {
					Bdist[v] = alt;
					if (Fprocessed[v]) {
						minDist = min (minDist, Bdist[Bu]+Fdist[v]+w);
					}
...
int main() {
	...
	for(int numCase=1;numCase<=nCases;numCase++) {
		...
		for(i=0;i<m;i++) {
			...
			adj[u][deg[u]++] = make_pair(w,v);
			adj[v][deg[v]++] = make_pair(w,u);

			//adj[u].emplace_back(make_pair(w,v));
			//adj[v].emplace_back(make_pair(w,u));
			//if(adj[u].size() > 7500) cout<<"fuck"<<endl; 
		}
