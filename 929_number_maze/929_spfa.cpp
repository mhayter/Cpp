...
//#define DEBUG
/*
int dijkstraCrazyQueue(int source, int n,int dest) {
	for(int i=0;i<n;i++) dist[i] = INF;
	
	queue<int> circularQue[10];
	int cirIndex=0;
	int nElements = 0;
...
void buildMyGraph(int nNodes) {
	//for(int i=0;i<nNodes;i++) myGraph[i].clear();
	//super node for begining
	deg[0]=0;
	myGraph[0][deg[0]++] = 1;
	...
	for(int i=0;i<nRows;i++) {
		for(int j=0;j<nCols;j++) {
			deg[nodeVal] = 0;
			if (j-1>=0) // add it
				myGraph[nodeVal][deg[nodeVal]++] = nodeVal-1;
			if (j+1<nCols) 
				myGraph[nodeVal][deg[nodeVal]++] = nodeVal+1;
			if (i-1>=0)
				myGraph[nodeVal][deg[nodeVal]++] = nodeVal-nCols;
			if (i+1<nRows)
				myGraph[nodeVal][deg[nodeVal]++] = nodeVal+nCols;
			++nodeVal;
		}
	...
	/*for(int i=0;i<nNodes;i++) {
		cerr << "Node i " << i << " connects  to " << endl;
...
int main() {
	...
	for(int caseNum=1;caseNum<=nCases;caseNum++) {
		...
		//fread(cString,1,nRows*(nCols) * 2,stdin);
		//int cStringIndex = 0;
		int nNodes = 1;
		for(int i=0;i<nRows;i++) {
			for(int j=0;j<nCols;j++) {
